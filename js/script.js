...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (coursesBody) {
        coursesData.forEach((item) => {
            ...
            bodyLines.push(`Kurs/Dienstleistung: ${item.type}`);
    ...
    // Formularverarbeitung: erzeugt eine Mail an die hinterlegte Adresse
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', (e) => {
            ...
            if (selectedValue) {
                bodyLines.push(`Ausgewählter Kurs/Termin: ${selectedValue}`);
            }
            ...
            // Leitet den Benutzer direkt zur E‑Mail‑Anwendung mit den ausgefüllten Daten weiter
            window.location.href = mailtoLink;

            // Formular zurücksetzen, damit neue Anfragen gestellt werden können
            bookingForm.reset();
        });
    }
});