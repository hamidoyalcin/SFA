...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (coursesBody) {
        coursesData.forEach((item) => {
            ...
            // Beim Klick wird zum Formular gescrollt und das Datum voreingestellt
            link.href = '#kursbuchung';
            // Speichern Sie den ausgewählten Termin im Datensatz des Links
            const optionValue = `${item.type} am ${item.date}`;
            link.dataset.optionValue = optionValue;
            link.addEventListener('click', (ev) => {
                const select = document.getElementById('selectedDate');
                if (select) {
                    // Nach dem passenden Wert suchen und auswählen
                    const value = ev.currentTarget.dataset.optionValue;
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', (e) => {
            ...
            if (contactMethod === 'whatsapp') {
                ...
                const whatsappNumber = '4915560881093';
